...
export const startPythonClient = async () => {
    ...
    // files = await readZipFile(new URL('./src/python/browser/stdlib.zip', window.location.href).href);
    files = await readZipFile(new URL('./src/python/browser/typeshed.zip', window.location.href).href);
    // init vscode-api
    await initServices({
        userServices: {
